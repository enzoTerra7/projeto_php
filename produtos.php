...
<script>
    function changePage(id) {
        document.cookie = 'produto_id_edit' + "=" + id + "; path=/";
        const a = document.createElement('a')
        a.href = `./produtos_edit.php`
        a.click()
    }

    function deleteProduct(id) {
        document.cookie = 'product_delete_id' + "=" + id + "; path=/";
        const a = document.createElement('a')
        a.href = `./services/produtos_delete.php`
        a.click()
    }
...
<body class="w-screen h-screen bg-white items-center justify-center">
                ...
                <main class="bg-neutral-200 p-4 md:p-8 gap-4 rounded-lg shadow-lg flex flex-col items-center container">
                    <div class="flex items-center justify-between w-full gap-4 flex-wrap">
                        ...
                        <span class="flex gap-4 flex-wrap">
                            <input id="filtro-nome" name="filtro-nome" type="text" required class="bg-neutral-100 p-2 rounded-lg min-w-[300px] border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira o nome do produto" />
                            <form action="./services/produtos_relatorio.php" method="post">
                    ...
                    if ($result) {
                        ...
                        echo '<th class="p-3 text-left">Quantidade em Estoque</th>';
                        echo '<th class="p-3 text-left">Valor Unitário</th>';
                        echo '<th class="p-3 text-left">Unidade de Medida</th>';
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo '<td class="border-grey-light border hover:bg-gray-100 p-3">' . $row['nome'] . '</td>';
                            echo '<td class="border-grey-light border hover:bg-gray-100 p-3">' . $row['qtd_estoque'] . '</td>';
                            echo '<td class="border-grey-light border hover-bg-gray-100 p-3">R$' . number_format($row['valor_unitario'], 2, ',', '.') . '</td>';
                            echo '<td class="border-grey-light border hover:bg-gray-100 p-3">' . $row['unidade_medida'] . '</td>';
                            echo '
    ...
    <script>
        ...
        filtroNome.addEventListener('input', filtrarPorNome);

        function filtrarPorNome() {
            const valorFiltro = filtroNome.value.toLowerCase().trim();
            ...
            linhasTabela.forEach(linha => {
                ...
                const correspondeFiltro = nome.includes(valorFiltro);
                linha.style.display = correspondeFiltro ? '' : 'none';
            });
        }
    </script>
