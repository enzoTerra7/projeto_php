...
if (isset($_POST['cadastrar'])) {
  ...
  $prazo_entrega = (int)$_POST['prazo_entrega'];
  $produtos = $_POST['selecionados'];

  if (!$conexao) {
    die("Falha na conexão com o banco de dados: " . mysqli_connect_error());
  }

  $query = "INSERT INTO pedidos (data, observacao, cond_pagto, prazo_entrega, id_cliente) 
              VALUES (?, ?, ?, ?, ?)";

  $stmt = mysqli_prepare($conexao, $query);

  if ($stmt) {
    mysqli_stmt_bind_param($stmt, 'sssid', $data, $observacao, $cond_pagto, $prazo_entrega, $id_cliente);

    if (mysqli_stmt_execute($stmt)) {
      $id_pedido = mysqli_insert_id($conexao);

      // Inserir itens_pedido para cada produto selecionado
      foreach ($produtos as $id_produto) {
        $qtde = 10; // Valor fixo
        $query_itens = "INSERT INTO itens_pedido (qtde, id_pedido, id_produto) VALUES (?, ?, ?)";
        $stmt_itens = mysqli_prepare($conexao, $query_itens);

        if ($stmt_itens) {
          mysqli_stmt_bind_param($stmt_itens, 'iii', $qtde, $id_pedido, $id_produto);
          mysqli_stmt_execute($stmt_itens);
          mysqli_stmt_close($stmt_itens);
        } else {
          echo "Erro na preparação da declaração para itens_pedidos: " . mysqli_error($conexao);
        }
      }
      ...
      echo "Erro ao cadastrar o pedido: " . mysqli_stmt_error($stmt);
    }
...
<head>
  ...
  <title>Ecom | Cadastrar pedido</title>
</head>
      ...
      <div class="mt-12 h-[calc(100vh-80px)] flex flex-col">
        <main class="bg-neutral-200 p-4 md:p-8 gap-4 rounded-lg shadow-lg flex flex-col items-center container">
          <div class="flex items-center justify-between w-full gap-4 flex-wrap">
            <h1 class="text-2xl font-bold">Cadastrar Pedidos</h1>

          </div>
          <hr class="border border-neutral-100 w-full" />
          <form class="w-full flex flex-col gap-4" method="POST" action="pedidos_view.php" onsubmit="return validarFormulario();">
            <div class="flex gap-4 flex-wrap items-center">
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="nome">Data</label>
                <input name="data" id="data" type="date" required class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Informe a data" />
              </div>
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="qtd_estoque">Observação</label>
                <input name="observacao" id="observacao" type="text" step="1" maxlength="255" required class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira as observações" />
              </div>
            </div>
            <div class="flex gap-4 flex-wrap items-center">
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="valor">Condição de Pagamento</label>
                <input name="cond_pagto" id="cond_pagto" type="varchar" required maxlength="255" class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Informe a condição de pagamento" />
              </div>
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="unit">Prazo de entrega em dias</label>
                <input name="prazo_entrega" id="prazo_entrega" type="number" step="1" required class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Informe o prazo de entrega" />
              </div>
            </div>
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                  echo '
                    <label class="border border-gray-300 rounded-lg shadow p-4 flex gap-4 items-center cursor-pointer flex-1 min-w-[300px]">
                      <input type="checkbox" name="selecionados[]" value="' . $row['id'] . '" class="p-2 border border-gray-300 rounded bg-transparent w-5 h-5" />
                      <div class="flex flex-col">
                        <h1 class="text-lg font-bold">' . $row['nome'] . '</h1>
                        <small>Ao selecionar, será adicionado 10 unidades ao pedido.</small>
                      </div>
                    </label>
                    ';
                  echo '</tr>';
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
              <div class="flex gap-2 w-fit items-center my-4">
                ...
                <button type="submit" name="cadastrar" class="bg-blue-500 text-white p-2 rounded-lg hover:bg-blue-700 transition-colors duration-300">
                  Salvar
                </button>
              </div>
            </div>
