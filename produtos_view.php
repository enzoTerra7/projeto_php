...
if (isset($_POST['cadastrar'])) {
  ...
  $query = "INSERT INTO produto (nome, qtd_estoque, valor_unitario, unidade_medida) 
              VALUES (?, ?, ?, ?)";

  $stmt = mysqli_prepare($conexao, $query);

  if ($stmt) {
    mysqli_stmt_bind_param($stmt, 'sdis', $nome, $qtd_estoque, $valor, $unit);

    if (mysqli_stmt_execute($stmt)) {
      header("Location: produtos.php");
    } else {
      echo "Erro ao cadastrar o produto: " . mysqli_stmt_error($stmt);
    }
...
<head>
  ...
  <title>Ecom | Cadastrar produto</title>
</head>
      ...
      <div class="mt-12 h-[calc(100vh-80px)] flex flex-col">
        <main class="bg-neutral-200 p-4 md:p-8 gap-4 rounded-lg shadow-lg flex flex-col items-center container">
          <div class="flex items-center justify-between w-full gap-4 flex-wrap">
            <h1 class="text-2xl font-bold">Cadastrar Produtos</h1>

          </div>
          <hr class="border border-neutral-100 w-full" />
          <form class="w-full flex flex-col gap-4" method="POST" action="produtos_view.php">
            <div class="flex gap-4 flex-wrap items-center">
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="nome">Nome</label>
                <input name="nome" id="nome" type="text" required maxlength="255" class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira o nome do produto" />
              </div>
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="qtd_estoque">Quantidade em Estoque</label>
                <input name="qtd_estoque" id="qtd_estoque" type="number" step="1" required class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira a quantidade do produto" />
              </div>
            </div>
            <div class="flex gap-4 flex-wrap items-center">
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="valor">Valor do Produto</label>
                <input name="valor" id="valor" type="number" required min="0" max="9999999999.99" step="0.01" class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira o Valor do produto" />
              </div>
              <div class="flex flex-col gap-2 min-w-[320px] flex-1 cursor-pointer">
                <label for="unit">Unidade de Medida</label>
                <input name="unit" id="unit" type="text" maxlength="10" required class="bg-neutral-100 p-2 rounded-lg w-full border-none outline-0 focus:ring-2 focus:ring-blue-500 cursor-pointer" placeholder="Insira a unidade de medida do produto" />
              </div>
            </div>
